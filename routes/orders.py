...
def create_order():
    ...
    order = Order(table_name=data['table_name'], cashier_id=identity, cashier_name=user.name,
                  waiter_name=data.get('waiter_name', user.name), status='pending',
                  total=data['total'], order_number=order_number, created_at=kenya_time())
...
def update_order(order_id):
    ...
    for item in data['items']:
        db.session.add(OrderItem(order_id=order_id, product_id=item['product_id'],
            product_name=item['product_name'], quantity=item['quantity'],
            price=item['price'], subtotal=item['subtotal']))
    db.session.commit()
...
def confirm_order(order_id):
    ...
    sale = Sale(cashier_id=identity, cashier_name=user.name, total=total,
                amount_paid=total, change_due=0,
                payment_method=order.payment_method or 'cash', created_at=kenya_time())
    db.session.add(sale)
    db.session.flush()
...
def serialize(o):
    return {
        'id': o.id, 'order_number': o.order_number, 'table_name': o.table_name,
        'waiter_name': o.waiter_name, 'cashier_name': o.cashier_name, 'status': o.status,
        'total': float(o.total), 'payment_method': o.payment_method,
        'payment_details': o.payment_details, 'rejection_note': o.rejection_note,
        ...
        'items': [{'product_id': i.product_id, 'product_name': i.product_name,
                   'quantity': i.quantity, 'price': float(i.price), 'subtotal': float(i.subtotal)}
                  for i in o.items]
    }
